...
|Standard general-purpose v1 do not support Private Endpoint    |Standard general-purpose v1 is not supported on private endpoints. General-purpose v1 accounts is a legacy storage account type and may not have the latest features or the lowest per-gigabyte pricing.  | You can upgrade a general-purpose v1 storage account to a general-purpose v2 account. Note that this action cannot be undone. For more information, see [Upgrade to a general-purpose v2 storage account](https://docs.microsoft.com/en-us/azure/storage/common/storage-account-upgrade?tabs=azure-portal).        |
